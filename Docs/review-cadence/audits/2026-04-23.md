...
Status: FIXED (2026-04-24, via PR — structured outputs wired with a strict `session_end_analysis` JSON schema; `parseAnalysisJson` deleted; explicit truncation + refusal handling added with distinct Sentry stage tags. Zod deferred — structured outputs + RPC defensive-parse provide two layers already. See live ledger for full detail.)
