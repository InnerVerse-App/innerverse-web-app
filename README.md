...
- Live prompts embed their pinned model in the filename (e.g., `prompt-X-gpt-5.4.md`). When you change the model in `src/lib/openai.ts`, rename the file to match.
  - `prompt-session-opener-gpt-5.4-mini.md` — focus-aware opening rules; governs the coach's first message only.
  - `prompt-v11.4-gpt-5.4.md` — master coaching prompt (operator-authored); governs every turn after the opener, sent verbatim.
  - `prompt-session-end-v7-gpt-5.4.md` — session-end analysis prompt (returns structured JSON).
  - `prompt-session-response-v2-gpt-5.4.md` — post-session reflection parser.
  - `prompt-growth-narrative-v1-gpt-5.4.md` — cumulative "Message from your Coach" letter.
  - `prompt-style-calibration-v1-gpt-5.4-mini.md` — feedback-driven style calibration aggregator.
...
- **OpenAI** `/v1/responses` for the coach (gpt-5.4 for chat / analyzer / response parser / narrative; gpt-5.4-mini for opener / calibration)
