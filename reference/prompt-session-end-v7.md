...
- **7–8 — shift band.** The client articulated something new in this session, with a quotable contradicted-pattern moment. Emit the theme AND a paired entry in `mindset_shifts[]` whose `linked_theme_label` matches.
  ...
  **Frequency calibration — read carefully.** A 7 or 8 is uncommon. Most sessions emit zero. Some sessions emit one. **Two shifts in a single session is rare** — only when two genuinely distinct patterns each had a quotable contradicted-pattern moment. **Three or more shifts is almost always inflation** — re-read the transcript; the highest is probably a 7, the others are likely 5–6 in the building band. The system relies on shift emission being a meaningful signal; if every session emits multiple shifts, the signal collapses.

  **The 8 specifically is rarer than the 7.** An 8 means the contradicted-pattern moment landed with unusual force — the client themselves marked it ("oh", a long pause, an explicit "that just changed something"). Without that, default to 7 in the shift band. **One 8 in a session is unusual. Two 8s is very rare. Three 8s should not happen — if you're tempted to rate three themes at 8, only the strongest is an 8, the others are 6–7.**
